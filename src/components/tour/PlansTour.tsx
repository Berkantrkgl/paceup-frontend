...
type TourStep = {
  text: string;
  target: "chatbot" | "planList" | null;
};

const STEPS: TourStep[] = [
  {
    text: "Buradan yapay zeka koçunla konuşarak\nyeni bir antrenman planı oluşturabilirsin.",
    target: "chatbot",
  },
  {
    text: "Oluşturduğun planlar burada listelenecek.\nBirden fazla plan oluşturabilirsin.",
    target: "planList",
  },
];

interface PlansTourProps {
  highlightRefs: {
    chatbot: React.RefObject<View | null>;
    planList: React.RefObject<View | null>;
  };
}

export const PlansTour = ({ highlightRefs }: PlansTourProps) => {
  ...
  useEffect(() => {
    if (user && user.is_onboarded && user.tour_plans === false) {
    ...
    try {
      ...
      if (validToken) {
        await fetch(`${API_URL}/users/me/`, {
          ...
          body: JSON.stringify({ tour_plans: true }),
        });
        await refreshUserData();
      }
    } catch (e) {
      console.log("[PlansTour] error:", e);
    }
  };
