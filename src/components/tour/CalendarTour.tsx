...
type TourStep = {
  text: string;
  target: "slider" | "calendar" | null;
};

const STEPS: TourStep[] = [
  {
    text: "Antrenmanlarını türüne göre\ntakvim üzerinden görüntüleyebilirsin.",
    target: "calendar",
  },
  {
    text: "Antrenmanların burada listelenecek.\nKaydırarak detaylarına bakabilirsin.",
    target: "slider",
  },
];
...
interface CalendarTourProps {
  highlightRefs: {
    slider: React.RefObject<View | null>;
    calendar: React.RefObject<View | null>;
  };
}

export const CalendarTour = ({ highlightRefs }: CalendarTourProps) => {
  ...
  useEffect(() => {
    if (user && user.is_onboarded && user.tour_calendar === false) {
      const timer = setTimeout(() => {
        setVisible(true);
        // Measure and show first step immediately
        measureAndShow(0);
    ...
    try {
      ...
      if (validToken) {
        await fetch(`${API_URL}/users/me/`, {
          ...
          body: JSON.stringify({ tour_calendar: true }),
        });
        await refreshUserData();
      }
    } catch (e) {
      console.log("[CalendarTour] error:", e);
    }
  };
